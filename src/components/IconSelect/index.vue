        ...
        <ul class="icon-list m-0 pl-10px">
          <template v-for="icon in filterIcons" :key="icon">
            <el-tooltip effect="dark" :content="icon" placement="top">
              <li class="icon-item p-5px mr-10px mb-10px w-10% cursor-pointer" @click="selectIcon(icon)">
                <svg-icon :name="icon" />
              </li>
            </el-tooltip>
          </template>
...
const filterIcons = computed(() => icons.filter((item) => item.includes(searchValue.value)))
// 选则图标
